#
# Get the beidou locations and put them in a separate table
...
# Drop and create the beidou table
my $sql = "drop table beidou;";
...
create table beidou (
             ...
             used    integer);
";
# select addgeometrycolumn('beidou','wkb_geometry', 4326, 'POINT', 2);
my $sth = $dbh->prepare($sql);  
$sth->execute();
$dbh->commit or print $DBI::errstr;

# Get the list
my $sqlsel = "select ogc_fid,time,wkb_geometry,ele,sat,hdop,vdop,pdop,cmt,st_x(wkb_geometry),st_y(wkb_geometry) from track_points where cmt like \'%bd2_used%\' and time is not null order by time asc;";
my $sthsel = $dbh->prepare($sqlsel);  
$sthsel->execute();
my $sqlins = "insert into beidou (ogc_fid,view,used) values (?,?,?);";
my $sthins = $dbh->prepare($sqlins);  

while (my @row = $sthsel->fetchrow_array ) {  
   my @values = ($row[8] =~ m/bd2_[a-z]+=(\S+)/g);
